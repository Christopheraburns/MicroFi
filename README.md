...
- [x] Fourteen embedded processors. **Nine** compile into the standalone build (the XIAO / generic-ESP32 set): `GenerateFlowFile`, `LogAttribute`, `UpdateAttribute`, `PublishMQTT`, `GetGPIO`, `SetGPIO`, `ListenHTTP`, `CaptureImage`, `PublishSparkplug`. **Five** AMOLED sensor/HMI processors ship in the tree behind `MICROFI_BOARD_*` guards and are selected by the `waveshare-devices` overlay build for the AMOLED class: `GetIMU`, `GetTouch`, `CaptureAudio`, `PlayAudio`, `DisplayMessage`. See [`docs/Processor-Inventory-And-Roadmap.md`](docs/Processor-Inventory-And-Roadmap.md#shipped) for what each does and how it maps to the roadmap.
